...
For example, all versions of BlinkInput In-browser SDK are available on Unpkg CDN:
...
1. Install BlinkInput In-browser SDK as NPM dependency with `npm install --save @microblink/blinkinput-in-browser-sdk`.

2. Setup Angular to automatically copy BlinkInput In-browser SDK assets to public location. Add the following code to `angular.json` inside `projects.<projectName>.architect.build.options.assets` array:

```JSON
{
  "glob": "**/*",
  "input": "node_modules/@microblink/blinkinput-in-browser-sdk/resources",
  "output": "/blinkinput-resources/"
}
```
...
```typescript
import { defineCustomElements } from '@microblink/blinkinput-in-browser-sdk/ui/loader';
...
5. Here's one possible way to use `<blinkinput-in-browser>` custom web component inside Angular:
...
@Component({
  ...
  template: '<blinkinput-in-browser #el></blinkinput-in-browser>',
  styleUrls: ['./my-component.component.scss']
})
export class MyComponent implements AfterViewInit {
  // Reference to the `blinkinput-in-browser` custom web component
  @ViewChild('el') el!: ElementRef<HTMLBlinkinputInBrowserElement>;

  constructor() {}

  ngAfterViewInit(): void {
    this.el.nativeElement.licenseKey = '<PLACE-YOUR-LICENSE-KEY-HERE>';
    this.el.nativeElement.recognizers = [ 'BarcodeRecognizer' ];

    // Engine location depends on the actual location of WebAssembly resources
    this.el.nativeElement.engineLocation = window.location.origin + '/blinkinput-resources/';
...
# Add `postinstall` hook to `package.json` that will copy resources
{
    ...
    "scripts": {
        ...
        "postinstall": "shx cp -r node_modules/@microblink/blinkinput-in-browser-sdk/resources public"
        ...
    },
    ...
}
...
3. Here's one possible way to use `<blinkinput-in-browser>` custom web component inside React:
...
import {
  applyPolyfills,
  defineCustomElements
} from '@microblink/blinkinput-in-browser-sdk/ui/loader';
...
function App() {
  ...
  React.useEffect(() => {
    applyPolyfills().then(() => {
      defineCustomElements().then(() => {
        ...
        el.current.recognizers = [ 'BarcodeRecognizer' ];
  ...
  return (
    <blinkinput-in-browser ref={el}></blinkinput-in-browser>
  );
}
...
* **Recognizer**
    * The `Recognizer` is the basic unit of processing within the BlinkInput SDK. Its main purpose is to process the image and extract meaningful information from it.
...
All attributes, properties and events of UI component can be seen in [`<blinkinput-in-browser>` API documentation](docs/components/blinkinput-in-browser/readme.md).
...
blinkinput-in-browser::part(mb-overlay) {
    background-color: green;
}

/* Change the background color of a specific overlay element */
blinkinput-in-browser::part(mb-overlay-camera-experience) {
...
For a full list of customizable icons, see [`<blinkinput-in-browser>` API documentation](docs/components/blinkinput-in-browser/readme.md).
